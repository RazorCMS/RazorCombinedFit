...
# from SingleBoxFit.RazorBox import getBinning
from prepareCombineWorkspace import getBinning
...
def findBTagProb(jets, efftype='data', SFerrdir=0, doLight=False, CFerrdir=0):
    ...
    for j in jets:
        ...
        if SFerrdir:
            if (not doLight and abs(j.partonFlavour) in [4, 5]) or \
               (doLight and not abs(j.partonFlavour) in [4, 5]):
...
def writeTree2DataSet(data, outputDir, outputFile, box, rMin, mRmin, label, args, jes_pe, pdf_pe, btag_pe, isr_pe, lep_pe, nominal, pdf_cen, jes_up, jes_down, pdf_up, pdf_down, btag_up, btag_down, isr_up, isr_down, lep_up, lep_down, mstop, mlsp):
    ...
    # for d in data:
    for histo in [nominal, pdf_cen, jes_up, jes_down, pdf_up, pdf_down,
                  btag_up, btag_down, isr_up, isr_down, lep_up, lep_down]:
        histo.Scale(weight)
    ...
    for histo in [jes_pe, pdf_pe, btag_pe, isr_pe, lep_pe, nominal, pdf_cen,
                  jes_up, jes_down, pdf_up, pdf_down, btag_up, btag_down,
                  isr_up, isr_down, lep_up, lep_down]:
    ...
    output = rt.TFile.Open(outputDir + '/' + outputFile + "_MR" + str(mRmin) +
                           "_R" + str(rMin) + '_' + str(mstop) + '_' +
                           str(mlsp) + '_' + box + '.root', 'RECREATE')
...
def convertTree2Dataset(tree, outputDir, outputFile, config, Min, Max, filter, run, mstop, mlsp, write=True):
    ...
    MRbins = array('d', getBinning(box, 'T2tt', False)[0])
    Rsqbins = array('d', getBinning(box, 'T2tt', False)[1])
    nBtagbins = array('d', getBinning(box, 'T2tt', False)[2])
    x = array('d', getBinning(box, 'T2tt', False)[0])
    y = array('d', getBinning(box, 'T2tt', False)[1])
    z = array('d', getBinning(box, 'T2tt', False)[2])

    # Book the histograms
    jes_pe = rt.TH3D("wHisto_JESerr_pe", "wHisto_JESerr_pe", len(MRbins)-1, x,
                     len(Rsqbins)-1, y, len(nBtagbins)-1, z)
    btag_pe = rt.TH3D("wHisto_btagerr_pe", "wHisto_btagerr_pe", len(MRbins)-1,
                      x, len(Rsqbins)-1, y, len(nBtagbins)-1, z)
    isr_pe = rt.TH3D("wHisto_ISRerr_pe", "wHisto_ISRerr_pe", len(MRbins)-1, x,
                     len(Rsqbins)-1, y, len(nBtagbins)-1, z)
    lep_pe = rt.TH3D("wHisto_LEPerr_pe", "wHisto_LEPerr_pe", len(MRbins)-1, x,
                     len(Rsqbins)-1, y, len(nBtagbins)-1, z)
    pdf_pe = rt.TH3D("wHisto_PDFerr_pe", "wHisto_PDFerr_pe", len(MRbins)-1, x,
                     len(Rsqbins)-1, y, len(nBtagbins)-1, z)

    jes_up = rt.TH3D("wHisto_JESerr_up", "wHisto_JESerr_up", len(MRbins)-1, x,
                     len(Rsqbins)-1, y, len(nBtagbins)-1, z)
    jes_down = rt.TH3D("wHisto_JESerr_down", "wHisto_JESerr_down", len(MRbins)-1, x, len(Rsqbins)-1, y, len(nBtagbins)-1, z)

    btag_up = rt.TH3D("wHisto_btagerr_up", "wHisto_btagerr_up", len(MRbins)-1,
                      x, len(Rsqbins)-1, y, len(nBtagbins)-1, z)
    btag_down = rt.TH3D("wHisto_btagerr_down", "wHisto_btagerr_down", len(MRbins)-1, x, len(Rsqbins)-1, y, len(nBtagbins)-1, z)

    isr_up = rt.TH3D("wHisto_ISRerr_up", "wHisto_ISRerr_up", len(MRbins)-1, x,
                     len(Rsqbins)-1, y, len(nBtagbins)-1, z)
    isr_down = rt.TH3D("wHisto_ISRerr_down", "wHisto_ISRerr_down", len(MRbins)-1, x, len(Rsqbins)-1, y, len(nBtagbins)-1, z)

    lep_up = rt.TH3D("wHisto_LEPerr_up", "wHisto_LEPerr_up", len(MRbins)-1, x,
                     len(Rsqbins)-1, y, len(nBtagbins)-1, z)
    lep_down = rt.TH3D("wHisto_LEPerr_down", "wHisto_LEPerr_down", len(MRbins)-1, x, len(Rsqbins)-1, y, len(nBtagbins)-1, z)

    nominal = rt.TH3D("wHisto", "wHisto", len(MRbins)-1, x, len(Rsqbins)-1, y,
                      len(nBtagbins)-1, z)
    ...
    # data.add(a)
        ...
        # DEBUG
        btagw_nominal = findBTagProb(jets, 'data')/bt_prob_fast  # assign a weight to each event w = P(data)/P(MC fast)
        btag_nominal = tree.nCSVM

        btag_SF_bc_up = findBTagProb(jets, efftype='data', SFerrdir=+1)/bt_prob_fast
        btag_SF_bc_Eup = abs(btagw_nominal - btag_SF_bc_up)/btag_SF_bc_up
        btag_SF_bc_dw = findBTagProb(jets, efftype='data', SFerrdir=-1)/bt_prob_fast
        btag_SF_bc_Edw = abs(btagw_nominal - btag_SF_bc_dw)/btag_SF_bc_dw

        btag_SF_lq_up = findBTagProb(jets, efftype='data', SFerrdir=+1, doLight=True)/bt_prob_fast
        btag_SF_lq_Eup = abs(btagw_nominal - btag_SF_lq_up)/btag_SF_lq_up
        btag_SF_lq_dw = findBTagProb(jets, efftype='data', SFerrdir=-1, doLight=True)/bt_prob_fast
        btag_SF_lq_Edw = abs(btagw_nominal - btag_SF_lq_dw)/btag_SF_lq_dw

        btag_CF_up = findBTagProb(jets, efftype='data', CFerrdir=+1)/bt_prob_fast
        btag_CF_Eup = abs(btagw_nominal - btag_CF_up)/btag_CF_up
        btag_CF_dw = findBTagProb(jets, efftype='data', CFerrdir=-1)/bt_prob_fast
        btag_CF_Edw = abs(btagw_nominal - btag_CF_dw)/btag_CF_dw

        btag_Eup = rt.TMath.Sqrt(btag_SF_bc_Eup**2 + btag_SF_lq_Eup**2 + btag_CF_Eup**2)*btagw_nominal
        btag_Edw = rt.TMath.Sqrt(btag_SF_bc_Edw**2 + btag_SF_lq_Edw**2 + btag_CF_Edw**2)*btagw_nominal

        btagw_up = btagw_nominal + btag_Eup
        btagw_dw = btagw_nominal - btag_Edw
        ...
        weight = 0.95
        lepw_nominal = 1.
        nominal.Fill(MR, RSQ, btag_nominal,
                     weight*btagw_nominal*lepw_nominal*isrw_nominal)

        # err
        jes_up.Fill(tree.MR_JES_UP, tree.RSQ_JES_UP, btag_nominal,
                    weight*btagw_nominal*lepw_nominal*isrw_nominal)
        jes_down.Fill(tree.MR_JES_DOWN, tree.RSQ_JES_DOWN, btag_nominal,
                      weight*btagw_nominal*lepw_nominal*isrw_nominal)
        btag_up.Fill(MR, RSQ, btag_nominal,
                     weight*btagw_up*lepw_nominal*isrw_nominal)
        btag_down.Fill(MR, RSQ, btag_nominal,
                       weight*btagw_dw*lepw_nominal*isrw_nominal)
        isr_up.Fill(MR, RSQ, btag_nominal,
                    weight*isrw_up*btagw_nominal*lepw_nominal)
        isr_down.Fill(MR, RSQ, btag_nominal,
                      weight*isrw_dw*btagw_nominal*lepw_nominal)
        lep_up.Fill(MR, RSQ, btag_nominal,
                    weight*lepw_up*btagw_nominal*isrw_nominal)
        lep_down.Fill(MR, RSQ, btag_nominal,
                      weight*lepw_dw*btagw_nominal*isrw_nominal)
        ...
        for icteq in range(45):
            vwHisto_pdfCTEQ[icteq].Fill(MR, RSQ, btag_nominal,
                                        weight*btagw_nominal*lepw_nominal *
                                        isrw_nominal*CTEQ66_W[icteq])
        for innpdf in range(101):
            vwHisto_pdfNNPDF[innpdf].Fill(MR, RSQ, btag_nominal,
                                          weight*btagw_nominal*lepw_nominal *
                                          isrw_nominal*NNPDF_W[innpdf])
    ...
    pdf_down = pdf_err.Clone("wHisto_pdferr_down")
    ...
    label = ""
    writeTree2DataSet(data, outputDir, outputFile, box, rMin, mRmin, label,
                      args, jes_pe, pdf_pe, btag_pe, isr_pe, lep_pe, nominal,
                      pdf_cen, jes_up, jes_down, pdf_up, pdf_down, btag_up,
                      btag_down, isr_up, isr_down, lep_up, lep_down, mstop,
                      mlsp)
...
if __name__ == '__main__':
    ...
    parser.add_option('-b', '--box', dest="box", type="string",
                      default="BJetHS",
                      help="box to run")
    parser.add_option('-e', '--eff', dest="eff", default=False,
                      action='store_true',
    ...
    if box == "BJetHS":
        ...
        convertTree2Dataset(chain, outputDir, fName, cfg, options.min,
                            options.max, BJetBoxLS(CalcBDT(chain)),
                            options.run, options.mstop, options.mlsp)
    if box == "Mu":
        convertTree2Dataset(chain, outputDir, fName, cfg, options.min,
