...
def binnedHisto2D(tree, binsX, binsY, label='') :
    ...
    for i_binx in range(0, len(binsX) - 1):
        for i_biny in range(0, len(binsY) - 1):
            ...
            numEntries = (tree.reduce("MR > %s && MR < %s && Rsq > %s && Rsq < %s && nBtag >= 1"%(binx, binx_p1, biny, biny_p1)).numEntries())
            print numEntries
...
if __name__ == '__main__':
    ...
    #MRbins = array('d',[350., 390., 430., 470.,   510., 550,  600.,   700.,800.,   1000.,4000.])
    #Rsqbins = array('d',[0.08,  0.10, 0.12,  0.14,  0.17,  0.20,  0.23, 0.26, 0.30,  0.40, 0.50,   0.7,  1.0,  1.5])
    #French flag plots binning
    MRbins = array('d',[350, 450, 550, 700, 900, 1200, 1600, 2500, 4000])
    Rsqbins = array('d',[0.08, 0.10, 0.15, 0.20,0.30,0.41,0.52,0.64,0.80,1.5])
    
    ###### SAMPLES
    #signal
    directory = '/afs/cern.ch/work/l/lucieg/public/forRazorStop/MC/T2ttEle4jets/mLSP' +str(mLSP)+'/'
