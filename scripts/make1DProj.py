    ...
    for k in range(1, nbinz):
        ...
        if switchToKDE:
            ...
            for j in range(0,len(nExp)):
                ...
                nExpList.add(nExp[j])
            dataset = rt.RooDataSet("dataset", "dataset", myTree, nExpSet)
            sumExp = rt.RooFormulaVar("sumExp", "sumExp", sumName[:-1],
                                      nExpList)
            sumExpData = dataset.addColumn(sumExp)
            sumExpData.setRange(0, maxX)
            rho = makeToyPVALUE_sigbin.useThisRho(0., maxX, myhisto)
            rkpdf = rt.RooKeysPdf("rkpdf", "rkpdf", sumExpData, dataset,
                                  rt.RooKeysPdf.NoMirror, rho)
            func = rkpdf.asTF(rt.RooArgList(sumExpData))
            mode, xmin, xmax, probRange, funcFill68 = \
                makeToyPVALUE_sigbin.find68ProbRangeFromKDEMode(maxX, func)
        else:
            xmax, xmin = GetProbRange(myhisto)
        print xmin, xmax
        err.append((xmax-xmin)/2.)
...
if __name__ == '__main__':
    ...
    for i in range(4, len(sys.argv)):
        ...
        if sys.argv[i].find("-Lumi=") != -1:
            Lumi = float(sys.argv[i].replace("-Lumi=", ""))
        if sys.argv[i].find("-Energy=") != -1:
            Energy = float(sys.argv[i].replace("-Energy=", ""))
    ...
    # TTj1b histograms
    hMRTTj1bList = [fitFile.Get("%s/histoToyTTj1b_MR_%s_ALLCOMPONENTS" %
                                (Box, frLabel)) for frLabel in frLabels]
    hRSQTTj1bList = [fitFile.Get("%s/histoToyTTj1b_Rsq_%s_ALLCOMPONENTS" %
                                 (Box, frLabel)) for frLabel in frLabels]
    if fit3D:
        hBTAGTTj1bList = [fitFile.Get("%s/histoToyTTj1b_nBtag_%s_ALLCOMPONENTS"
                                      % (Box, frLabel)) for frLabel in frLabels]

    # TTj2b histograms
    hMRTTj2bList = [fitFile.Get("%s/histoToyTTj2b_MR_%s_ALLCOMPONENTS" %
                                (Box, frLabel)) for frLabel in frLabels]
    hRSQTTj2bList = [fitFile.Get("%s/histoToyTTj2b_Rsq_%s_ALLCOMPONENTS" %
                                 (Box, frLabel)) for frLabel in frLabels]
    if fit3D:
        hBTAGTTj2bList = [fitFile.Get("%s/histoToyTTj2b_nBtag_%s_ALLCOMPONENTS"
                                      %(Box, frLabel)) for frLabel in frLabels]

    # Vpj histograms
    hMRVpjList = [fitFile.Get("%s/histoToyVpj_MR_%s_ALLCOMPONENTS" %
                              (Box, frLabel)) for frLabel in frLabels]
    hRSQVpjList = [fitFile.Get("%s/histoToyVpj_Rsq_%s_ALLCOMPONENTS" %
                               (Box, frLabel)) for frLabel in frLabels]
    if fit3D:
        hBTAGVpjList = [fitFile.Get("%s/histoToyVpj_nBtag_%s_ALLCOMPONENTS" %
                                    (Box, frLabel)) for frLabel in frLabels]

    # Total Bkg histograms
    hMRTOTList = [fitFile.Get("%s/histoToy_MR_%s_ALLCOMPONENTS" %
                              (Box, frLabel)) for frLabel in frLabels]
    hRSQTOTList = [fitFile.Get("%s/histoToy_Rsq_%s_ALLCOMPONENTS" %
                               (Box, frLabel)) for frLabel in frLabels]
    if fit3D:
        hBTAGTOTList = [fitFile.Get("%s/histoToy_nBtag_%s_ALLCOMPONENTS" %
                                    (Box, frLabel)) for frLabel in frLabels]

    # Data histograms
    hMRDataList = [fitFile.Get("%s/histoData_MR_%s_ALLCOMPONENTS" %
                               (Box, frLabel)) for frLabel in frLabels]
    hRSQDataList = [fitFile.Get("%s/histoData_Rsq_%s_ALLCOMPONENTS" %
                                (Box, frLabel)) for frLabel in frLabels]
    if fit3D:
        hBTAGDataList = [fitFile.Get("%s/histoData_nBtag_%s_ALLCOMPONENTS" %
                                     (Box, frLabel)) for frLabel in frLabels]

    # Signal histograms
    hMRSignalList = [fitFile.Get("%s/histoToySignal_MR_%s_ALLCOMPONENTS" %
                                 (Box, frLabel)) for frLabel in frLabels]
    hRSQSignalList = [fitFile.Get("%s/histoToySignal_Rsq_%s_ALLCOMPONENTS" %
                                  (Box, frLabel)) for frLabel in frLabels]
    if fit3D:
        hBTAGSignalList = [fitFile.Get("%s/histoToySignal_nBtag_%s_"
                                       "ALLCOMPONENTS" %
                                       (Box, frLabel)) for frLabel in frLabels]
    ...
    for hMRTOT, hMRTTj1b, hMRTTj2b, hMRVpj, hMRData, hMRSignal, hRSQTOT, hRSQTTj1b, hRSQTTj2b, hRSQVpj, hRSQData, hRSQSignal, hBTAGTOT, hBTAGTTj1b, hBTAGTTj2b, hBTAGVpj, hBTAGData, hBTAGSignal, btagOpt, frLabel in zip(hMRTOTList, hMRTTj1bList, hMRTTj2bList, hMRVpjList, hMRDataList,  hMRSignalList, hRSQTOTList, hRSQTTj1bList, hRSQTTj2bList, hRSQVpjList, hRSQDataList, hRSQSignalList,  hBTAGTOTList, hBTAGTTj1bList, hBTAGTTj2bList, hBTAGVpjList, hBTAGDataList, hBTAGSignalList, btagToDo, frLabel):
        ...
        if btagOpt == 0:
            ...
            for i in range(1, len(errBTAG)+1):
                hBTAGTOTcopy.SetBinError(i, max(errBTAG[i-1],
                                         hBTAGTOT.GetBinError(i)))
        ...
        if fit3D and btagOpt == 0:
            goodPlot("BTAG", Box, outFolder, Label, Energy, Lumi, hBTAGTOTcopy, hBTAGTOT, hBTAGTTj1b, hBTAGTTj2b, hBTAGVpj, hBTAGData,  hBTAGSignal, c1, pad1, pad2, fit3D, btagOpt)
