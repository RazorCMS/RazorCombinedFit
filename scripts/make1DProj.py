...
def goodPlot(varname, outFolder, Label, Energy, Lumi, hMRTOTcopy, hMRTOT, hMRTTj1b, hMRTTj2b, hMRData, c1, pad1, pad2, fit3D, btag):
    ...
    hMRData.Sumw2()
    hMRTOTcopy.Sumw2()
    hMRTOTcopycopy = hMRTOTcopy.Clone(hMRTOTcopy.GetName()+"Divide") 
    hMRDataDivide = hMRData.Clone(hMRData.GetName()+"Divide")
    hMRDataDivide.Sumw2()

    hMRTOTcopycopy.GetYaxis().SetTitle("")
    hMRTOTcopycopy.GetYaxis().SetLabelSize(0.12)
    hMRTOTcopycopy.SetTitle("")
    if varname=="BTAG": hMRTOTcopycopy.GetXaxis().SetLabelSize(0.28)
    else: hMRTOTcopycopy.GetXaxis().SetLabelSize(0.18)
    hMRTOTcopycopy.GetXaxis().SetTitleSize(0.18)
    hMRTOTcopycopy.GetXaxis().SetTitleOffset(0.85)
 
    #hMRDataDivide.Divide(hMRTOTcopy)
    for i in range(1, hMRData.GetNbinsX()+1):
        tmpVal = hMRTOTcopycopy.GetBinContent(i)
        if tmpVal != -0.:
            ...
            hMRTOTcopycopy.SetBinContent(i, hMRTOTcopycopy.GetBinContent(i)/tmpVal)
            hMRTOTcopycopy.SetBinError(i, hMRTOTcopycopy.GetBinError(i)/tmpVal)

    hMRTOTcopycopy.SetLineWidth(2)
    hMRTOTcopycopy.SetLineColor(rt.kBlue+1)
    hMRTOTcopycopy.SetFillColor(rt.kBlue+1)
    hMRTOTcopycopy.SetMarkerColor(rt.kBlue+1)
    hMRTOTcopycopy.SetMarkerStyle(21)
    hMRTOTcopycopy.SetMarkerSize(1)
    hMRTOTcopycopy.SetFillStyle(1001)
    hMRTOTcopycopy.GetYaxis().SetNdivisions(504,rt.kTRUE)
    hMRTOTcopycopy.Draw("e2")
    hMRDataDivide.Draw('pesame')

    leg.AddEntry(hMRDataDivide,"Data/Prediction","lep")
    leg.AddEntry(hMRTOTcopycopy,"Prediction Uncertainty")
    c1.cd()
