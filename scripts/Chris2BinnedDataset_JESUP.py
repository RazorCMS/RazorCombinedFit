...
def convertTree2Dataset(tree, outputFile, outputBox, config, box, minH, maxH, nToys, write = True):
    ...
    tree.Project("wHisto_xsecdown", "RSQ_JES_UP:MR_JES_UP",  'LEP_W*W_DOWN*(MR_JES_UP >= %f && MR_JES_UP <= %f && RSQ_JES_UP >= %f && RSQ_JES_UP <= %f && (BOX_NUM == %i))' % (mRmin,mRmax,rsqMin,rsqMax,boxMap[box]))
