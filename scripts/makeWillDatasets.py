    ...
    for mLSP in range(25, 50, 25):  # 725
        ...
        for mStop in range(mLSP+100, 825, 25):
            ...
            print ("python scripts/Will2DatasetwithSYS.py --box %s -c %s "
                   "--mlsp=%s --mstop=%s -d %s root://osg-se.cac.cornell.edu//"
            ...
            runScript.write('xrdcp root://osg-se.cac.cornell.edu//xrootd/path/'
                            'cms/store/user/salvati/Razor/MultiJet2012/'
                            'CMSSW_5_3_14/RMRTrees/T2tt/SMS-T2tt_mStop-Combo.'
                            '0_8TeV-Pythia6Z-Summer12-START52_V9_FSIM-v1-SUSY.'
                            'pkl /tmp/SMS-T2tt_mStop-Combo.0_8TeV-Pythia6Z-'
                            'Summer12-START52_V9_FSIM-v1-SUSY.pkl\n')
            runScript.write('python scripts/Will2DatasetwithSYS.py --box %s '
                            '-c %s --mlsp=%s --mstop=%s -d /tmp/ root://osg-se.'
                            'cac.cornell.edu//xrootd/path/cms/store/user/'
                            'salvati/Razor/MultiJet2012/CMSSW_5_3_14/RMRTrees/'
                            'T2tt/SMS-T2tt_mStop-Combo_mLSP_%s.0_8TeV-Pythia6Z-'
