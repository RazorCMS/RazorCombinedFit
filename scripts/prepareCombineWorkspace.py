...
def getTTjBinEvents(i, j, k, x, y, z, workspace):
    errorFlag = False
    if z[k-1] == 3:
        bkg = "TTj2b"
    else:
    ...
    ymin = y[0]
    ymax = y[-1]
    total_integral = (N / rt.TMath.Power(B*N, N)) * \
        (Gfun(xmin, ymin, X0, Y0, B, N)-Gfun(xmin, ymax, X0, Y0, B, N) -
         Gfun(xmax, ymin, X0, Y0, B, N)+Gfun(xmax, ymax, X0, Y0, B, N))

    xmin = x[i-1]
    xmax = x[i]
    ymin = y[j-1]
    ymax = y[j]
    integral = (N / rt.TMath.Power(B*N, N)) * \
        (Gfun(xmin, ymin, X0, Y0, B, N)-Gfun(xmin, ymax, X0, Y0, B, N) -
         Gfun(xmax, ymin, X0, Y0, B, N)+Gfun(xmax, ymax, X0, Y0, B, N))
...
def getVpjBinEvents(i, j, k, x, y, z, workspace):
    errorFlag = False
    bkg = "Vpj"
    B = workspace.var("b_%s" % bkg).getVal()
    N = workspace.var("n_%s" % bkg).getVal()
...
def getBinning(box, model, coarse=True):
    ...
    if box in ["Ele", "Mu"]:
        if coarse:
            ...
            Rsqbins = [0.08, 0.10, 0.15, 0.20, 0.30,
                       0.41, 0.52, 0.64, 0.80, 1.5]
            # MRbins = [350., 370., 390., 410., 430., 450., 470., 490., 510.,
            #           530., 550, 575., 600., 625., 650., 700., 800., 900.,
            #           1000., 4000.]
            # Rsqbins = [0.08, 0.09, 0.10, 0.11, 0.12, 0.13, 0.14, 0.15, 0.17,
            #            0.20, 0.23, 0.26, 0.30, 0.35, 0.40, 0.50, 0.7, 1.0, 1.5]
        ...
        if coarse:
            ...
            # 72 bins
            # MRbins = [450., 550., 800., 4000.]
            # Rsqbins = [0.10, 0.70, 1.5]

            # MRbins = [450, 490., 530., 575., 625., 700., 900., 4000.]
            # Rsqbins = [0.10, 0.12, 0.14, 0.17, 0.23, 0.30, 0.40, 0.7, 1.5]
            # MRbins = [450., 470., 510., 550, 600., 800., 1000., 4000.]
            # Rsqbins = [0.10, 0.15, 0.20, 0.26, 0.35, 0.50, 1.0, 1.5]
            # MRbins = [450., 600., 750., 900., 1200., 1600., 4000.]
            # Rsqbins = [0.10, 0.13, 0.20, 0.30, 0.41, 0.52, 0.64, 1.5]
        else:
            # MRbins = [450., 460., 470., 490., 510., 530., 550., 600., 700.,
            #           800., 1000., 4000.]
            # Rsqbins = [0.12, 0.15, 0.20, 0.30, 0.50, 0.70, 1.1, 1.5]

            # 126 bins: 6x7x3
            MRbins = [450, 600, 750, 900, 1200, 1600, 4000]
            Rsqbins = [0.10, 0.13, 0.20, 0.30, 0.41, 0.52, 0.64, 1.5]

            # MRbins = [450, 525, 600, 750, 900, 1200, 1600, 4000]
            # Rsqbins = [0.10, 0.115, 0.13, 0.165, 0.20,
            #            0.30, 0.41, 0.52, 0.64, 1.5]

            # 180 nbins
            # MRbins = [450., 470., 490., 510., 530., 550., 600., 700.,
            #           800., 1000., 4000.]
            # Rsqbins = [0.10, 0.20, 0.30, 0.50, 0.70, 1.1, 1.5]

            # 72 bins
            # MRbins = [450., 500., 550, 600., 800., 1000., 4000.]
            # Rsqbins = [0.10, 0.35, 0.70, 1.1, 1.5]

            # MRbins = [450., 470., 490., 510., 530., 550, 575., 600., 625.,
            #           650., 700., 800., 900., 1000., 4000.]
            # Rsqbins = [0.10, 0.11, 0.12, 0.13, 0.14, 0.15, 0.17,
            #            0.20, 0.23, 0.26, 0.30, 0.35, 0.40, 0.50, 0.7, 1.0, 1.5]
            # MRbins = [450., 500., 600., 675., 750., 825., 900., 1000., 1200.,
            #           1400., 1600., 2500., 4000.]
            # Rsqbins = [0.10, 0.11, 0.13, 0.16, 0.20, 0.25,
            #            0.30, 0.35, 0.41, 0.46, 0.52, 0.58, 0.64, 1.0, 1.5]
...
def passCut(MRVal, RsqVal, box, signalRegion):
    ...
    if box in ["Ele", "Mu"]:
        if signalRegion == "FULL":
            if MRVal >= 350. and RsqVal >= 0.08:
                passBool = True
        elif signalRegion == "Sideband":
            if (MRVal >= 350. and RsqVal >= 0.08 and MRVal < 450.) or (MR >= 350. and Rsq >= 0.08 and RsqVal < 0.13):
                passBool = True
        ...
        if signalRegion == "FULL":
            ...
            if (MRVal >= 450. and MRVal < 600 and RsqVal >= 0.10) or (MRVal >= 450. and RsqVal >= 0.10 and RsqVal < 0.13):
                passBool = True
...
def writeDataCard(box, model, massPoint, txtfileName, bkgs, paramNames, w,
                  lumi_uncert, trigger_uncert, lepton_uncert, penalty):
    ...
    for paramName in paramNames:
        if paramName.find("Ntot") != -1 or paramName.find("f3") != -1:
            continue
...
if __name__ == '__main__':
    ...
    if refXsecFile is not None:
        ...
        print "INFO: ref xsec taken to be: %s mass %d, xsec = %f fb" % \
            (gluinoHistName, mGluino, refXsec)
    ...
    # 3D histograms -> build histos with
    # expected(fit -> component)/observed(data)/signal events per bin
    histos = {}
    histos_coarse = {}
    histos[box, model] = rt.TH3D("%s_%s_3d" % (box, model), "%s_%s_3d" %
                                 (box, model), len(x)-1, x, len(y)-1, y,
                                 len(z)-1, z)
    histos[box, "data"] = rt.TH3D("%s_%s_3d" % (box, "data"), "%s_%s_3d" %
                                  (box, "data"), len(x)-1, x, len(y)-1, y,
                                  len(z)-1, z)

    histos_coarse[box, model] = rt.TH3D("%s_%s_coarse" % (box, model),
                                        "%s_%s_coarse" %
                                        (box, model), len(x)-1, x, len(y)-1, y,
                                        len(z)-1, z)
    ...
    for bkg in initialBkgs:
        histos[box, bkg] = rt.TH3D("%s_%s_3d" % (box, bkg), "%s_%s_3d" %
                                   (box, bkg), len(x)-1, x, len(y)-1, y,
                                   len(z)-1, z)
            ...
            for j in xrange(1, len(y)):
                for k in xrange(1, len(z)):
                    if not passCut(x[i-1], y[j-1], box, signalRegion):
                        continue
                    if bkg is "Vpj" and z[k-1] > 1:
                        continue

                    if bkg is "Vpj":
                        bin_events, errorFlag = getVpjBinEvents(i, j, k, x, y,
                                                                z, workspace)
                    else:
                        bin_events, errorFlag = getTTjBinEvents(i, j, k, x, y,
                                                                z, workspace)

                    if (bkg.find("1b") != -1 or bkg.find("Vpj") != -1)\
                            and z[k-1] == 1:
                        # we always have k=z[k-1]=1
                        histos[box, bkg].SetBinContent(i, j, k, bin_events)
                        # print "bkg, k, z[k-1], bin_events:",\
                        #     bkg, k, z[k-1], bin_events
    ...
    # Building the Razor pdf : parameters, cuts, pdf -> TTj2b (from fit)
    # turns into TTj2b (2 btags -> 1-f3), TTj3b (3 btags-> f3)
    emptyHist3D = {}
    emptyHist3D[box] = rt.TH3D("EmptyHist3D_%s" % (box), "EmptyHist3D_%s" %
                               (box), len(x)-1, x, len(y)-1, y, len(z)-1, z)
    ...
    for paramName in paramNames:
        ...
        w.var("%s_%s" % (paramName, box)).setError(workspace.var(paramName).
                                                   getError())
    ...
    if box in ["BJetHS", "BJetLS"]:
        w.factory("%s_%s[%e]" % (workspace.var("n_Vpj").GetName(), box,
                                 workspace.var("n_Vpj").getVal()))
        print workspace.var("n_Vpj").getVal()
        w.var("%s_%s" % (workspace.var("n_Vpj").GetName(), box)).\
            setConstant(True)
    ...
    razorPdf_TTj1b = rt.RooRazor3DBinPdf("%s_%s" % (box, "TTj1b"),
                                         ...
                                         w.var("th1x"),
                                         w.var("MR0_%s_%s" % ("TTj1b", box)),
                                         w.var("R0_%s_%s" % ("TTj1b", box)),
                                         w.var("b_%s_%s" % ("TTj1b", box)),
                                         w.var("n_%s_%s" % ("TTj1b", box)),
                                         w.var("MRCut_%s" % (box)),
                                         w.var("RCut_%s" % (box)),
                                         w.var("BtagCut_%s" % ("TTj1b")),
    ...
    razorPdf_TTj2b = rt.RooRazor3DBinPdf("%s_%s" % (box, "TTj2b"),
                                         ...
                                         w.var("th1x"),
                                         w.var("MR0_%s_%s" % ("TTj2b", box)),
                                         w.var("R0_%s_%s" % ("TTj2b", box)),
                                         w.var("b_%s_%s" % ("TTj2b", box)),
                                         w.var("n_%s_%s" % ("TTj2b", box)),
                                         w.var("MRCut_%s" % (box)),
                                         w.var("RCut_%s" % (box)),
                                         w.var("BtagCut_%s" % ("TTj2b")),
                                         w.obj("EmptyHist3D_%s" % (box)))
    ...
    w.var("%s_%s_norm" % (box, "TTj3b")).\
        setError(w.var("Ntot_TTj2b_%s" % box).getError() *
                 w.var("f3_TTj2b_%s" % box).getVal())
    extRazorPdf_TTj3b = rt.RooExtendPdf("ext%s_%s" % (box, "TTj3b"),
                                        "extRazorPdf_%s_%s" % (box, "TTj3b"),
                                        razorPdf_TTj3b,
                                        w.var("%s_TTj3b_norm" % box))
    ...
    if box in ["BJetHS", "BJetLS"]:
        razorPdf_Vpj = rt.RooRazor3DBinPdf("%s_%s" % (box, "Vpj"),
                                           ...
                                           w.var("th1x"),
                                           w.var("MR0_%s_%s" % ("Vpj", box)),
                                           w.var("R0_%s_%s" % ("Vpj", box)),
                                           w.var("b_%s_%s" % ("Vpj", box)),
                                           w.var("n_%s_%s" % ("Vpj", box)),
                                           w.var("MRCut_%s" % (box)),
                                           w.var("RCut_%s" % (box)),
                                           w.var("BtagCut_%s" % ("Vpj")),
                                           w.obj("EmptyHist3D_%s" % (box)))
        w.factory("%s_%s_norm[%f,0,1e6]" % (box, "Vpj",
                                            w.var("Ntot_Vpj_%s" % box).getVal()))
        RootTools.Utils.importToWS(w, razorPdf_Vpj)


        extRazorPdf_Vpj = rt.RooExtendPdf("ext%s_%s" % (box, "Vpj"),
                                          "extRazorPdf_%s_%s" % (box, "Vpj"),
                                          razorPdf_Vpj,
                                          w.var("%s_Vpj_norm" % box))
    ...
    # histos[(box, "%s_IsrUp" % (model))], histos_coarse[(box, "%s_IsrUp" % (model))] = \
    #     rebin3dCoarse(isrUp, xSignal, ySignal, zSignal, x, y, z, box,
    #                   signalRegion)
    # histos[(box,"%s_IsrDown"%(model))], NULL = rebin3dCoarse(isrDown, xSignal, ySignal ,zSignal, x,y,z, box, signalRegion)

    # histos[(box,"%s_BtagUp"%(model))], NULL = rebin3dCoarse(btagUp, xSignal, ySignal ,zSignal, x,y,z,box, signalRegion)
    # histos[(box,"%s_BtagDown"%(model))], NULL = rebin3dCoarse(btagDown,xSignal, ySignal ,zSignal, x,y,z,box, signalRegion)

    # histos[(box,"%s_JesUp"%(model))], NULL = rebin3dCoarse(jesUp,xSignal, ySignal ,zSignal, x,y,z, box, signalRegion)
    # histos[(box,"%s_JesDown"%(model))], NULL = rebin3dCoarse(jesDown,xSignal, ySignal ,zSignal, x,y,z, box, signalRegion)

    # histos[(box,"%s_PdfUp"%(model))], NULL = rebin3dCoarse(pdfUp,xSignal, ySignal ,zSignal, x,y,z,box, signalRegion)
    # histos[(box,"%s_PdfDown"%(model))], NULL = rebin3dCoarse(pdfDown, xSignal, ySignal ,zSignal, x,y,z,box, signalRegion)
    ...
    print ("\nINFO: now multiplying: efficiency x lumi x refXsec = %f x %f x "
           "%f = %f") % (sigNorm, lumi, refXsec, sigEvents)

    # histos[box, model], histos_coarse[box, model] = rebin3dCoarse(wHisto.Clone("%s_%s_3d" % (box, model)),
    #                                    xSignal, ySignal, zSignal, x, y, z, box,
    #                                    signalRegion)
    histos[box, model] = wHisto.Clone("%s_%s_3d" % (box, model))
    histos[box, model].SetTitle("%s_%s_3d" % (box, model))
    histos[box, model].Scale(sigEvents/histos[box, model].Integral())
    # histos_coarse[box, model].Scale(sigEvents/histos_coarse[box, model].Integral())

    for paramName in ["Jes", "Isr", "Btag", "Pdf"]:
        print ("\nINFO: Now renormalizing signal shape systematic histograms "
               "to nominal\n")
        print "signal shape variation %s" % paramName

        for syst in ['Up', 'Down']:
            print "norm is %f" % histos[box, "%s_%s%s" % (model, paramName,
                                                          syst)].Integral()
            if histos[box, "%s_%s%s" % (model, paramName,
                                        syst)].Integral() > 0:
                histos[box, "%s_%s%s" % (model, paramName, syst)].\
                    Scale(histos[box, model].Integral()/histos
                          [box, "%s_%s%s" % (model, paramName, syst)].
                          Integral())
    ...
    for index, histo in histos.iteritems():
        ...
        # print box, bkg
        if bkg == "data":
            histos1d[box, bkg] = rt.TH1D("data_obs", "data_obs",
                                         nBins, 0, nBins)
        else:
            histos1d[box, bkg] = rt.TH1D("%s_%s" % (box, bkg),
                                         "%s_%s" % (box, bkg), nBins, 0, nBins)
        ...
        for i in xrange(1, histo.GetNbinsX()+1):
            for j in xrange(1, histo.GetNbinsY()+1):
                for k in xrange(1, histo.GetNbinsZ()+1):
                    ...
                    histos1d[box, bkg].\
                        SetBinContent(newbin, histo.GetBinContent(i, j, k))

        if bkg == "data":
            dataHist[box, bkg] =\
                rt.RooDataHist("data_obs", "data_obs",
                               th1xList, rt.RooFit.Import(histos1d[box, bkg]))
            RootTools.Utils.importToWS(w, dataHist[box, bkg])

        # ? histos1d for sig do not have the same binning (coarser)
        elif bkg.find("TTj") == -1 and bkg.find("Vpj") == -1:
            dataHist[box, bkg] =\
                rt.RooDataHist("%s_%s" % (box, bkg), "%s_%s" % (box, bkg),
                               th1xList, rt.RooFit.Import(histos1d[box, bkg]))
            RootTools.Utils.importToWS(w, dataHist[box, bkg])

    print "\nINFO: Now writing data card\n"
    w.Print("v")
    writeDataCard(box, model, massPoint, "%s/razor_combine_%s_%s_%s_%s.txt" %
                  (outdir, box, njets, model, massPoint), initialBkgs,
                  paramNames, w, lumi_uncert, trigger_uncert, lepton_uncert,
                  penalty)
    ...
    outFile = rt.TFile.Open("%s/razor_combine_%s_%s_%s_%s.root" %
                            (outdir, box, njets, model, massPoint), "RECREATE")
    outFile.cd()

    for index, histo in histos.iteritems():
        histo.Write()
    for index, histo in histos_coarse.iteritems():
        histo.Write()
