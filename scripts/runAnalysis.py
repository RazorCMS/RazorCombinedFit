...
def defineParser():
    ...
    parser.add_option('--save-toys', dest="save_toys", action="store_true",
                      default=False,
                      help="Save the toys as text files for future use.")
    parser.add_option('--save-toys-from-fit', dest="save_toys_from_fit",
    ...
    parser.add_option('--likelihood-scan', dest="likelihood_scan",
                      default=False, action='store_true',
                      help="Run the likelihood scan")
    parser.add_option('--pulls', dest="pulls", default=False,
                      action='store_true',
                      help="Run the pulls fit")
    parser.add_option('-l', '--limit', dest="limit", default=False,
                      action='store_true',
                      help="Run the model-dependent limit setting code")
    parser.add_option('--study', dest="study", default=False,
                      action='store_true',
                      help="Run a simple RooMCStudy")
    parser.add_option('-e', '--expected-limit', dest="expectedlimit",
                      default=False, action='store_true',
                      help="Run the model-dependent bkg-only toy MC to compute the expected limit")
    parser.add_option('-x', '--xsec', dest="signal_xsec", type="float",
                      default=-99,
    ...
    parser.add_option('--fit-region', dest="fitregion", type="string",
                      default='FULL',
                      help="Perform the fit in the selected region: FULL, Sideband")
    parser.add_option('--nosave-workspace', dest="nosave_workspace",
                      default=False, action='store_true',
                      help="Do not save the RooWorkspaces to save disk space for limit setting")
    parser.add_option('--fitmode', dest="fitMode", type="string",
                      default='3D', help="Type of fit to run - 2D, 3D, 4D")
...
if __name__ == '__main__':
    ...
    if options.analysis is not None:
        a = [SingleBoxFit.SingleBoxAnalysis(options.output, cfg, analysis)]
        for aa in a:
