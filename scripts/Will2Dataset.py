...
boxMap = {'MuEle': 0, 'MuMu': 1, 'EleEle': 2, 'Mu': 3, 'Ele': 4, 'Had': 5,
          'BJet': 6, 'BJetLS': 7, 'BJetHS': 8}
cross_sections = {'SingleTop_s': 4.21, 'SingleTop_t': 64.6,
                  'SingleTop_tw': 10.6, 'TTj': 157.5, 'Zll': 3048,
                  'Znn': 2*3048, 'Wln': 31314,
...
class BJetBox(object):
    ...
    def __call__(self, tree):
        return tree.hadBoxFilter and tree.nJet >= 6 and tree.nCSVM > 0 and\
            tree.MR >= MR_CUT_HAD and tree.RSQ >= RSQ_CUT and\
            tree.nMuonTight < 1\
            and tree.nElectronTight < 1 and not tree.isolatedTrack10Filter and\
            tree.nMuonLoose < 1 and tree.nElectronLoose < 1 and\
            tree.hadTriggerFilter
...
if __name__ == '__main__':
    ...
    parser.add_option('-x', '--box', dest="box", default="BJetHS",
    ...
    for f in args:
        if f.lower().endswith('.root'):
            ...
            print "File '%s' of unknown type. Looking for .root files only" % f

    if box == 'BJetHS':
        convertTree2Dataset(chain, fName, cfg, options.min, options.max,
                            BJetBoxHS(CalcBDT(chain)), options.run)
    elif box == 'BJetLS':
        convertTree2Dataset(chain, fName, cfg, options.min, options.max,
                            BJetBoxLS(CalcBDT(chain)), options.run)
    elif box == 'Ele':
        convertTree2Dataset(chain, fName, cfg, options.min, options.max,
                            EleBox(None), options.run)
    elif box == 'Mu':
        convertTree2Dataset(chain, fName, cfg, options.min, options.max,
                            MuBox(None), options.run)
