...
Double_t RooBTagMult::analyticalIntegral(Int_t code, const char* rangeName) const{
  ...
  if(code == 1) {
    if (xmin>=1. && xmin <=2.){
      if (xmax>1. && xmax <=2.){
	binVol1 = xmax-xmin; binVol2 = 0.; binVol3 = 0.;
	  }
      else if (xmax>2. && xmax<=3.){
	binVol1 = 1.; binVol2 = xmax-2.; binVol3 = 0; 
	  }
      else if (xmax>3. && xmax<=4.){
	binVol1 = 1.; binVol2 = 1.; binVol3 = xmax-3.;
	  }
    }

    else if (xmin>=2. && xmin <=3.){
      if (xmax>2. && xmax <=3.){
	binVol1 = 0.; binVol2 = xmax-xmin; binVol3 = 0.;
	  }
      else if (xmax>3. && xmax<=4.){
	binVol1 = 0.; binVol2 = 1.; binVol3 = xmax-3.;
	  }
    }

    else if (xmin>=3. && xmin <=4.){
      if (xmax>3. && xmax<=4.){
	binVol1 = 0.; binVol2 = 0.; binVol3 = xmax-xmin;
	  }
    }
